...
class Solution {
    public boolean searchMatrix(int[][] matrix, int target) {
        ...
        return binarySearch(matrix, row, target, 0, n);
    }
    
    boolean binarySearch(int[][] matrix, int row, int target, int start, int end) {
        if(start>end) {
            return false;
        }
        ...
        if(matrix[row][mid] == target) {
            ...
            return binarySearch(matrix, row, target, start, mid-1);
        } else {
            return binarySearch(matrix, row, target, mid+1, end);
        }
    }
}
